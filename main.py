...
def add_product_to_cart():
    ...
    try:
        ...
        if _quantity and _code and request.method == 'POST':
            ...
            #all_total_price = format(all_total_price, '.2f')
            #print('all_total_price after format is', all_total_price)
            session['all_total_price'] = all_total_price
            
            checksumstr = f"pid={pid:s}&sid={sid:s}&amount={all_total_price:.1f}&token={secret:s}"
            print('checksumstr is', checksumstr)
            #checksumstr = f"pid={pid:s}&sid={sid:s}&amount={all_total_price:.2f}"
            #print('pid is', pid)
            #print('sid is', sid)
            print('amount is', all_total_price)
            #print('token is', secret)
            #print('checksumstr is', checksumstr)
